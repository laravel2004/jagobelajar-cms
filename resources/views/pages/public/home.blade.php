            ...
            <div class="mt-8 grid gap-6 md:grid-cols-2 lg:mt-10 lg:grid-cols-3">
                @foreach (($programCopy ?: [[
                    ...
                    <article class="flex flex-col overflow-hidden rounded-2xl border-t-4 {{ $program['border'] ?? 'border-[#0043c6]' }} bg-white p-8 shadow-[0_10px_32px_rgba(20,27,44,0.08)] {{ $loop->iteration === 2 ? 'lg:-translate-y-2' : '' }}">
                        ...
                        <a href="{{ $program['url'] ?? ($homeContent->hero_primary_cta_url ?? route('materials.index')) }}" class="mt-8 inline-flex w-full items-center justify-center rounded-xl border-2 {{ $program['border'] ?? 'border-[#0043c6]' }} py-3 text-sm font-bold {{ $program['color'] ?? 'text-[#0043c6]' }} transition hover:bg-[#f1f3ff]">{{ $program['button'] ?? 'Lihat Program' }}</a>
                    </article>
                @endforeach
            </div>
        </div>
