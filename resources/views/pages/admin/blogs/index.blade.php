<x-layouts.admin :title="'Blog CMS - '.config('app.name')">
    <div class="space-y-6">
        @if (session('status'))
        ...
        <section class="overflow-hidden rounded-[2rem] bg-white shadow-[0_18px_55px_rgba(20,27,44,0.08)] ring-1 ring-[#e6eaf5]">
            <div class="relative bg-[radial-gradient(circle_at_85%_10%,rgba(254,183,0,0.35),transparent_28%),linear-gradient(135deg,#0b2f8f,#0043c6_48%,#1e5af0)] p-6 text-white sm:p-8">
                <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                    ...
                    <a href="{{ route('admin.blogs.create') }}" class="inline-flex items-center justify-center rounded-2xl bg-[#feb700] px-5 py-3 text-sm font-extrabold text-[#271900] transition hover:bg-[#ffca35]">Tambah Blog</a>
                </div>
            </div>
            ...
            <div class="divide-y divide-[#e9edff]">
                @forelse ($posts as $post)
                    <div class="flex flex-col gap-4 p-4 sm:p-6 md:flex-row md:items-center md:justify-between">
                        <div class="min-w-0">
                            <p class="text-xs font-bold uppercase tracking-[0.16em] text-[#8a93a8]">{{ $post->published_at?->translatedFormat('d F Y') ?? '-' }} · {{ $post->reading_minutes }} menit</p>
                            <h2 class="mt-2 text-xl font-extrabold text-[#141b2c]">{{ $post->title }}</h2>
                            <p class="mt-2 max-w-3xl text-sm leading-6 text-[#5f667d]">{{ $post->excerpt }}</p>
                        </div>
                        <div class="flex flex-wrap gap-3">
                            <a href="{{ route('admin.blogs.edit', $post) }}" class="rounded-xl bg-[#f1f3ff] px-4 py-2 text-sm font-bold text-[#0043c6]">Edit</a>
                            <form method="POST" action="{{ route('admin.blogs.destroy', $post) }}" onsubmit="return confirm('Hapus blog ini?')">
                                @csrf
                                @method('DELETE')
