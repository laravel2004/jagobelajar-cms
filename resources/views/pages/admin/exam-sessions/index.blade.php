    ...
    <div class="space-y-6">
        <section class="overflow-hidden rounded-[2rem] bg-white shadow-[0_18px_55px_rgba(20,27,44,0.08)] ring-1 ring-[#e6eaf5]">
            ...
            <div class="grid gap-px bg-[#e9edff] grid-cols-1 sm:grid-cols-2 xl:grid-cols-4">
                <div class="bg-white p-5"><p class="text-xs font-bold uppercase tracking-[0.18em] text-[#8a93a8]">Total Sesi</p><p class="mt-2 text-3xl font-extrabold text-[#141b2c]">{{ $totalSessions }}</p></div>
                <div class="bg-white p-5"><p class="text-xs font-bold uppercase tracking-[0.18em] text-[#8a93a8]">Published</p><p class="mt-2 text-3xl font-extrabold text-emerald-600">{{ $publishedCount }}</p></div>
                <div class="bg-white p-5"><p class="text-xs font-bold uppercase tracking-[0.18em] text-[#8a93a8]">Draft</p><p class="mt-2 text-3xl font-extrabold text-amber-500">{{ $draftCount }}</p></div>
                <div class="bg-white p-5"><p class="text-xs font-bold uppercase tracking-[0.18em] text-[#8a93a8]">Inactive</p><p class="mt-2 text-3xl font-extrabold text-slate-500">{{ $inactiveCount }}</p></div>
            </div>
        </section>
        ...
        <section class="rounded-[2rem] bg-white p-4 shadow-[0_14px_45px_rgba(20,27,44,0.06)] ring-1 ring-[#e6eaf5] sm:p-5">
            <div class="mb-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex items-center gap-3 w-full sm:w-auto">
                    <form action="{{ route('admin.exam-sessions.index') }}" method="GET" class="relative flex-1 sm:min-w-[300px]">
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari sesi ujian..." class="w-full rounded-2xl border border-[#e6eaf5] bg-[#f9f9ff] py-2.5 pl-10 pr-10 text-sm text-[#141b2c] transition focus:border-[#0043c6] focus:bg-white focus:outline-none focus:ring-1 focus:ring-[#0043c6]">
                        ...
                        @if (request('search'))
                            <a href="{{ route('admin.exam-sessions.index') }}" class="absolute right-3.5 top-1/2 -translate-y-1/2 text-[#8a93a8] hover:text-[#141b2c]">
                    ...
                    <span class="hidden sm:inline-flex rounded-full bg-[#f1f3ff] px-4 py-2 text-xs font-bold uppercase tracking-[0.16em] text-[#0043c6] whitespace-nowrap">{{ $examSessions->total() }} Item</span>
                </div>
            </div>
