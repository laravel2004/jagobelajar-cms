<x-layouts.admin>
    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.exam-bundles.create') }}" class="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-700">
        ...
        <div class="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ number_format($totalBundles) }}</p>
                </div>
            </div>
        </div>
        <div class="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ number_format($publishedCount) }}</p>
                </div>
            </div>
        </div>
        <div class="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ number_format($draftCount) }}</p>
                </div>
            </div>
        </div>
        <div class="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ number_format($inactiveCount) }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        <div class="border-b border-gray-200 bg-gray-50 p-4 flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <form action="{{ route('admin.exam-bundles.index') }}" method="GET" class="relative w-full sm:w-64">
                <input type="text" name="search" value="{{ $search }}" placeholder="Cari bundle..." class="w-full rounded-lg border border-gray-300 pl-10 py-2 pr-4 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm text-gray-600">
                ...
                <tbody class="divide-y divide-gray-200 bg-white">
                    @forelse ($examBundles as $bundle)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-4">
                                <div class="font-medium text-gray-900">{{ $bundle->title ?? $bundle->name }}</div>
                                <div class="text-xs text-gray-500 font-mono mt-1">{{ $bundle->slug }}</div>
                            </td>
                            <td class="px-6 py-4">
                                @if ($bundle->is_promo_active && $bundle->sale_price !== null && $bundle->sale_price < $bundle->price)
                                    <div class="text-sm font-bold text-emerald-600">Rp{{ number_format($bundle->sale_price, 0, ',', '.') }}</div>
                                    <div class="text-xs text-gray-400 line-through">Rp{{ number_format($bundle->price, 0, ',', '.') }}</div>
                                @else
                                    <div class="text-sm font-medium text-gray-900">
                                        {{ $bundle->price > 0 ? 'Rp'.number_format($bundle->price, 0, ',', '.') : 'Gratis' }}
                                    </div>
                                @endif
                            </td>
                            ...
                            <td class="px-6 py-4 text-xs whitespace-nowrap">
                                <div class="flex items-center gap-1.5">
                                    ...
                                    <span>{{ $bundle->updated_at?->format('d/m/Y H:i') ?? '-' }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <a href="{{ route('admin.exam-bundles.edit', $bundle) }}" class="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-xs font-medium text-gray-700 shadow-sm transition hover:bg-gray-50 hover:text-blue-600">
