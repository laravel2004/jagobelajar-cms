    ...
    <section class="bg-[#f9f9ff] py-12 sm:py-16">
        <div class="jb-container space-y-8">
                ...
                <div x-data="{ show: true }" x-show="show" class="rounded-[2rem] bg-emerald-50 p-5 shadow-[0_14px_40px_rgba(16,185,129,0.12)] ring-1 ring-emerald-100">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="mt-1 text-sm leading-6 text-emerald-800">{{ session('status') }}</p>
                        </div>
            ...
            <div class="rounded-[2rem] bg-[radial-gradient(circle_at_85%_12%,rgba(254,183,0,0.35),transparent_28%),linear-gradient(135deg,#0043c6,#1e5af0)] p-6 text-white shadow-[0_18px_50px_rgba(20,27,44,0.14)] sm:p-8">
                ...
                <h1 class="mt-3 text-3xl font-extrabold sm:text-4xl">Halo, {{ $user->name }}</h1>
                ...
                <div class="mt-5 grid gap-3 text-sm sm:grid-cols-3">
                    <div class="rounded-2xl bg-white/12 p-4 ring-1 ring-white/15"><span class="block text-white/70">Email</span><strong>{{ $user->email }}</strong></div>
                    <div class="rounded-2xl bg-white/12 p-4 ring-1 ring-white/15"><span class="block text-white/70">Whatsapp</span><strong>{{ $user->whatsapp ?? '-' }}</strong></div>
                    <div class="rounded-2xl bg-white/12 p-4 ring-1 ring-white/15"><span class="block text-white/70">Paket Terdaftar</span><strong>{{ $registeredPackages->count() }}</strong></div>
                </div>
                ...
                <div class="mt-6 rounded-[2rem] bg-white/12 p-5 ring-1 ring-white/15 backdrop-blur-sm">
                    <div class="flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <span class="rounded-full bg-white/15 px-3 py-1 text-xs font-bold uppercase tracking-[0.16em] text-white ring-1 ring-white/20">{{ $registeredPackages->count() }} Paket</span>
                    </div>
                    <div class="mt-4 grid gap-3">
                        @forelse ($registeredPackages as $package)
                            <div class="flex flex-col gap-3 rounded-2xl bg-white/12 p-4 ring-1 ring-white/15 transition hover:bg-white/15 sm:flex-row sm:items-center sm:justify-between">
                                <div class="flex items-start gap-3">
                                    <span class="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-white/15 text-sm font-extrabold text-white ring-1 ring-white/20">{{ strtoupper(substr($package->package_type, 0, 1)) }}</span>
                                    <div>
                                        <p class="font-extrabold text-white">{{ $package->package_name }}</p>
                                        <p class="mt-1 text-sm text-white/70">{{ ucfirst($package->package_type) }} - {{ ucfirst($package->status) }}</p>
                                    </div>
                                </div>
                                <div class="flex flex-col items-start gap-2 sm:items-end">
                                    <div class="text-sm text-white/80 sm:text-right">
                                        <p class="font-bold text-[#feb700]">{{ optional($package->registered_at)->format('d M Y') ?? '-' }}</p>
                                        <p class="mt-1 text-xs uppercase tracking-[0.16em] text-white/55">Tanggal Daftar</p>
                                    </div>
                                    @if ($package->join_url)
                                        <a href="{{ $package->join_url }}" target="_blank" rel="noopener noreferrer" class="inline-flex justify-center rounded-xl bg-white px-4 py-2 text-xs font-extrabold text-[#0043c6] transition hover:bg-[#f1f3ff]">{{ $package->package_type === 'bimbel' ? 'Masuk ke Forum' : 'Masuk ke Ujian' }}</a>
                                    @endif
                                </div>
                            </div>
                        @empty
            ...
            <section class="rounded-[2rem] bg-white p-6 shadow-[0_14px_40px_rgba(20,27,44,0.08)] ring-1 ring-[#e6eaf5]">
                <div class="flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <a href="{{ route('tryout.index') }}" class="text-sm font-bold text-[#0043c6]">Lihat semua</a>
                </div>
                <div class="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                    @forelse ($tryoutPackages as $package)
                        ...
                        <a href="{{ route('tryout.detail', $package->slug) }}" class="overflow-hidden rounded-3xl bg-white shadow-[0_14px_34px_rgba(20,27,44,0.10)] ring-1 ring-[#e9edff] transition hover:-translate-y-1">
                            <div class="relative aspect-[16/9] bg-[radial-gradient(circle_at_75%_25%,rgba(254,183,0,0.55),transparent_30%),linear-gradient(135deg,#0043c6,#1e5af0)] text-white">
                                @if ($package->image_path)
                                    <img src="{{ asset('storage/'.$package->image_path) }}" alt="{{ $package->title ?? $package->name }}" class="h-full w-full object-cover">
                                @else
                                    <div class="grid h-full w-full place-items-center text-white">
                                        <span class="rounded-2xl bg-white/15 px-5 py-3 text-3xl font-extrabold ring-1 ring-white/25">{{ strtoupper(substr($package->title ?? $package->name, 0, 2)) }}</span>
                                    </div>
                                @endif
                                <span class="absolute left-4 top-4 rounded-full bg-[#feb700] px-3 py-1 text-xs font-bold text-[#271900]">{{ $displayPrice > 0 ? 'Rp'.number_format($displayPrice, 0, ',', '.') : 'Gratis' }}</span>
                            </div>
                            <div class="p-5">
                                <p class="text-xs font-bold uppercase tracking-[0.18em] text-[#0043c6]">{{ optional($package->starts_at)->translatedFormat('d M Y') ?? '-' }}</p>
                                <h3 class="mt-2 text-lg font-extrabold text-[#141b2c]">{{ $package->title ?? $package->name }}</h3>
                                <p class="mt-2 line-clamp-2 text-sm leading-6 text-[#5f667d]">{{ \Illuminate\Support\Str::limit(strip_tags($package->description ?: 'Try out aktif yang bisa kamu akses di dashboard.'), 90) }}</p>
                            </div>
                        </a>
                    @empty
