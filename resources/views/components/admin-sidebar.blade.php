...
<aside id="admin-sidebar" class="fixed left-0 top-0 z-50 h-screen w-[86vw] max-w-72 -translate-x-full border-r border-[#e6eaf5] bg-white shadow-[24px_0_60px_rgba(20,27,44,0.16)] transition-transform duration-200 lg:translate-x-0 lg:shadow-none">
    <div class="flex h-full flex-col">
        ...
        <nav class="flex flex-col gap-2 overflow-y-auto px-4 py-4 text-sm font-semibold">
            ...
            <a class="{{ request()->routeIs('admin.dashboard') ? $activeLink : $inactiveLink }}" href="{{ route('admin.dashboard') }}">Dashboard</a>
            <a class="{{ request()->routeIs('admin.cms-beranda.*') ? $activeLink : $inactiveLink }}" href="{{ route('admin.cms-beranda.edit') }}">CMS Beranda</a>
            <a class="{{ request()->routeIs('admin.features.*') ? $activeLink : $inactiveLink }}" href="{{ route('admin.features.index') }}">CMS Keunggulan</a>
