        ...
        <section class="overflow-hidden rounded-[2rem] bg-white shadow-[0_18px_55px_rgba(20,27,44,0.08)] ring-1 ring-[#e6eaf5]">
            <div class="border-b border-[#e6eaf5] bg-white p-4 sm:p-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex items-center gap-3 w-full sm:w-auto">
                    <form action="<?php echo e(route('admin.users.index')); ?>" method="GET" class="relative flex-1 sm:min-w-[300px]">
                        <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari nama atau email..." class="w-full rounded-2xl border border-[#e6eaf5] bg-[#f9f9ff] py-2.5 pl-10 pr-10 text-sm text-[#141b2c] transition focus:border-[#0043c6] focus:bg-white focus:outline-none focus:ring-1 focus:ring-[#0043c6]">
                        ...
                        <?php if(request('search')): ?>
                            <a href="<?php echo e(route('admin.users.index')); ?>" class="absolute right-3.5 top-1/2 -translate-y-1/2 text-[#8a93a8] hover:text-[#141b2c]">
                    ...
                    <span class="hidden sm:inline-flex rounded-full bg-[#f1f3ff] px-4 py-2 text-xs font-bold uppercase tracking-[0.16em] text-[#0043c6] whitespace-nowrap"><?php echo e($users->total()); ?> User</span>
                </div>
            </div>
                                ...
                                <div x-show="showDeleteModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block px-4 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-2xl shadow-xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6 relative z-10">
                                    <div class="sm:flex sm:items-start">
                                        ...
                                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                            ...
                                            <div class="mt-2">
                                                <p class="text-sm text-slate-500">Apakah Anda yakin ingin menghapus user <strong><?php echo e($user->name); ?></strong>? Semua data yang terkait dengan user ini akan dihapus secara permanen. Aksi ini tidak dapat dibatalkan.</p>
                                            </div>
                                        </div>
                                    </div>
...
<?php endif; ?>
<?php /**PATH /Users/laravel2004/dev/jagobelajar-cms/resources/views/pages/admin/users/index.blade.php ENDPATH**/ ?>