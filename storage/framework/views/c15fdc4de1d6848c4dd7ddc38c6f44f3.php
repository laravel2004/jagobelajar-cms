...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.public','data' => ['title' => 'Jago Belajar - Jadi Juara di Setiap Ujian']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('Jago Belajar - Jadi Juara di Setiap Ujian')]); ?>
    <section class="relative overflow-hidden bg-[#f9f9ff]">
        ...
        <div class="jb-container relative grid min-h-[auto] items-center gap-10 py-12 sm:py-16 lg:min-h-[690px] lg:grid-cols-[0.88fr_1.12fr] lg:gap-10 lg:py-24">
            <div class="max-w-2xl text-center lg:text-left">
                <span class="inline-flex max-w-full items-center gap-2 rounded-full bg-[#dce1ff] px-4 py-2 text-left text-[11px] font-semibold text-[#003ab1] shadow-sm sm:text-xs">
                    ...
                    <?php echo e($homeContent->hero_badge ?? 'Bimbel & Try Out Online #BelajarJadiJago'); ?>

                </span>

                <h1 class="mt-6 text-3xl font-extrabold leading-tight tracking-[-0.04em] text-[#0043c6] sm:text-5xl lg:text-[56px] lg:leading-[1.06]">
                    <?php echo e($homeContent->hero_title ?? 'Raih Prestasi Terbaikmu Bersama Jago Belajar'); ?>

                </h1>

                <p class="mx-auto mt-5 max-w-xl text-sm leading-7 text-[#434655] sm:text-base lg:mx-0">
                    <?php echo e($homeContent->hero_description ?? 'Bimbel dan try out online untuk siswa SD - SMP - SMA | TKA | OSN. Bimbel online interaktif dibimbing tutor berpengalaman, materi dan latihan soal terstruktur, akses try out premium, analisis hasil belajar.'); ?>

                </p>

                <div class="mx-auto mt-7 grid max-w-xl gap-3 text-left text-sm font-semibold text-[#141b2c] sm:grid-cols-2 lg:mx-0">
                    <div class="flex items-center gap-3"><span class="grid h-5 w-5 place-items-center rounded-full bg-[#0043c6] text-[11px] text-white">✓</span><?php echo e($homeContent->hero_benefits[0] ?? 'Bimbel Online Seru'); ?></div>
                    <div class="flex items-center gap-3"><span class="grid h-5 w-5 place-items-center rounded-full bg-[#0043c6] text-[11px] text-white">✓</span><?php echo e($homeContent->hero_benefits[1] ?? 'Try Out Berkualitas'); ?></div>
                    <div class="flex items-center gap-3"><span class="grid h-5 w-5 place-items-center rounded-full bg-[#0043c6] text-[11px] text-white">✓</span><?php echo e($homeContent->hero_benefits[2] ?? 'Tutor Berpengalaman'); ?></div>
                    <div class="flex items-center gap-3"><span class="grid h-5 w-5 place-items-center rounded-full bg-[#0043c6] text-[11px] text-white">✓</span><?php echo e($homeContent->hero_benefits[3] ?? 'Analisis Hasil Belajar'); ?></div>
                </div>

                <div class="mt-8 flex flex-col justify-center gap-3 sm:flex-row lg:justify-start">
                    <a href="<?php echo e($homeContent->hero_primary_cta_url ?? route('materials.index')); ?>" class="inline-flex items-center justify-center gap-2 rounded-xl bg-[#feb700] px-7 py-4 text-sm font-bold text-[#271900] shadow-[0_14px_24px_rgba(254,183,0,0.25)] transition hover:-translate-y-0.5 hover:bg-[#ffca35]">
                        <?php echo e($homeContent->hero_primary_cta_label ?? 'Daftar Bimbel'); ?>

                        <span aria-hidden="true">→</span>
                    </a>
                    <a href="<?php echo e($homeContent->hero_secondary_cta_url ?? '#program'); ?>" class="inline-flex items-center justify-center rounded-xl border-2 border-[#0043c6] bg-white/70 px-7 py-4 text-sm font-bold text-[#0043c6] transition hover:-translate-y-0.5 hover:bg-[#dce1ff]">
                        <?php echo e($homeContent->hero_secondary_cta_label ?? 'Lihat Paket Belajar'); ?>

                    </a>
                </div>
            </div>

            <div class="relative mx-auto mt-6 hidden w-full max-w-[620px] justify-center md:flex md:max-w-[760px] lg:mt-0 lg:max-w-[980px] lg:-mr-16 lg:justify-self-end xl:-mr-24">
                ...
                <img
                    src="<?php echo e($homeContent->hero_image_path ? asset($homeContent->hero_image_path) : asset('images/no-bg-hero.png')); ?>"
                    alt="<?php echo e($homeContent->hero_image_alt ?? 'Siswa Jago Belajar'); ?>"
    ...
    <section id="keunggulan" class="bg-white py-12 sm:py-16">
        <div class="jb-container">
            <div class="text-center">
                <h2 class="text-2xl font-bold tracking-tight text-[#141b2c] sm:text-3xl"><?php echo e($homeContent->feature_title ?? 'Kenapa Memilih Jago Belajar?'); ?></h2>
            ...
            <div class="mx-auto mt-8 grid max-w-5xl gap-5 sm:grid-cols-2 lg:mt-10 lg:grid-cols-3 lg:justify-items-center">
                <?php $__currentLoopData = ($featureItems ?: [
                    ...
                    <article class="w-full max-w-[320px] rounded-2xl bg-white p-5 text-center shadow-[0_8px_26px_rgba(20,27,44,0.08)] ring-1 ring-[#e9edff] transition hover:-translate-y-1">
                        <div class="mx-auto grid h-12 w-12 place-items-center rounded-xl <?php echo e($tone); ?>">
                            <?php if($icon === 'chat'): ?>
                            ...
                            <?php elseif($icon === 'center_focus_strong'): ?>
                        ...
                        <h3 class="mt-4 text-sm font-bold text-[#141b2c]"><?php echo e($title); ?></h3>
                        <p class="mt-2 text-xs leading-5 text-[#434655]"><?php echo e($desc); ?></p>
                    </article>
    ...
    <section id="program" class="bg-[#eef2ff] py-16">
        <div class="jb-container">
            <div class="text-center">
                <h2 class="text-2xl font-bold tracking-tight text-[#141b2c] sm:text-3xl"><?php echo e($homeContent->program_title ?? 'Program Lengkap Jago Belajar'); ?></h2>
                <p class="mt-2 text-[#434655]"><?php echo e($homeContent->program_description ?? 'Semua kebutuhan belajarmu ada di sini!'); ?></p>
            </div>

            <div class="mt-8 grid gap-6 md:grid-cols-2 lg:mt-10 lg:grid-cols-3">
                <?php $__currentLoopData = ($programCopy ?: [[
                ...
                ]]); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $program): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    ...
                    <article class="flex flex-col overflow-hidden rounded-2xl border-t-4 <?php echo e($program['border'] ?? 'border-[#0043c6]'); ?> bg-white p-8 shadow-[0_10px_32px_rgba(20,27,44,0.08)] <?php echo e($loop->iteration === 2 ? 'lg:-translate-y-2' : ''); ?>">
                        <h3 class="text-lg font-bold <?php echo e($program['color'] ?? 'text-[#0043c6]'); ?>"><?php echo e($program['title']); ?></h3>
                        <p class="mt-1 text-xs font-bold uppercase tracking-wider text-[#434655]"><?php echo e($program['subtitle']); ?></p>
                        <ul class="mt-7 space-y-5">
                            <?php $__currentLoopData = $programPoints; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $point): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <li class="flex gap-3">
                                    <span class="grid h-9 w-9 shrink-0 place-items-center rounded-lg <?php echo e($program['background'] ?? 'bg-[#dce1ff]'); ?> <?php echo e($program['color'] ?? 'text-[#0043c6]'); ?>">
                                    ...
                                    <span>
                                        <span class="block text-sm font-bold text-[#141b2c]"><?php echo e($point['title']); ?></span>
                                        <span class="mt-1 block text-xs text-[#434655]"><?php echo e($point['description']); ?></span>
                                    </span>
                                </li>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </ul>
                        <a href="<?php echo e($homeContent->hero_primary_cta_url ?? route('materials.index')); ?>" class="mt-8 inline-flex w-full items-center justify-center rounded-xl border-2 <?php echo e($program['border'] ?? 'border-[#0043c6]'); ?> py-3 text-sm font-bold <?php echo e($program['color'] ?? 'text-[#0043c6]'); ?> transition hover:bg-[#f1f3ff]"><?php echo e($program['button'] ?? 'Lihat Program'); ?></a>
                    </article>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>
        </div>
    ...
    <section class="bg-[#f9f9ff] py-12 sm:py-16">
        <div class="jb-container">
            <div class="text-center">
                <h2 class="text-2xl font-bold tracking-tight text-[#141b2c] sm:text-3xl"><?php echo e($homeContent->gallery_title ?? 'Dokumentasi JagoanBelajar Seru'); ?></h2>
                <div class="mx-auto mt-3 h-1 w-16 rounded-full bg-[#feb700]"></div>
            </div>
            <div x-data="gallerySlider()" x-init="start()" @mouseenter="stop()" @mouseleave="start()" class="mt-8 lg:mt-10">
                <div class="relative overflow-hidden rounded-2xl">
                    <div class="flex transition-transform duration-700 ease-out" :style="`transform: translateX(-${active * 100}%);`">
                        <?php $__currentLoopData = ($galleryItems ?: [['label' => 'BIMBEL ONLINE', 'badge' => 'Live Class', 'image' => null], ['label' => 'TRY OUT', 'badge' => 'Premium', 'image' => null], ['label' => 'KELAS SERU', 'badge' => 'Interaktif', 'image' => null]]); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="w-full shrink-0">
                                <div class="group overflow-hidden rounded-xl bg-[#141b2c] shadow-[0_12px_30px_rgba(20,27,44,0.12)]">
                                    <div class="relative aspect-[16/9] overflow-hidden">
                                        <img src="<?php echo e(!empty($item['image']) ? asset($item['image']) : asset('images/no-bg-hero.png')); ?>" alt="<?php echo e($item['label']); ?>" class="h-full w-full object-cover">
                                        <div class="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-transparent"></div>
                                        <div class="absolute left-4 top-4 rounded-full bg-white/90 px-3 py-1 text-xs font-bold text-[#141b2c]"><?php echo e($item['badge']); ?></div>
                                        <div class="absolute inset-x-0 bottom-0 p-5 text-white">
                                            <p class="text-lg font-bold sm:text-xl"><?php echo e($item['label']); ?></p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="mt-4 flex justify-center gap-2">
                    <?php $__currentLoopData = ($galleryItems ?: [1,2,3]); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <button type="button" @click="goTo(<?php echo e($index); ?>)" :class="active === <?php echo e($index); ?> ? 'bg-[#0043c6] w-6' : 'bg-[#c3c5d8] w-2'" class="h-2 rounded-full transition-all"></button>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
            </div>
        </div>
    ...
    <section id="testimoni" class="bg-[#f9f9ff] py-12 sm:py-16">
        <div class="jb-container">
            <div class="text-center">
                <h2 class="text-2xl font-bold tracking-tight text-[#141b2c] sm:text-3xl"><?php echo e($homeContent->testimonials_title ?? 'Apa Kata Mereka?'); ?></h2>
                <div class="mx-auto mt-3 h-1 w-16 rounded-full bg-[#feb700]"></div>
            </div>
                        ...
                        <?php $__currentLoopData = ($testimonialItems ?: [['name' => 'Syafiq', 'role' => 'Siswa Jago Belajar', 'quote' => 'Matematika 70, B. Indonesia 86,67. Makasih atas bimbingannya!', 'image' => null], ['name' => 'Naufal', 'role' => 'Siswa Jago Belajar', 'quote' => 'Mtk n B. Indo semua 83,33. Terima kasih ilmunya kak!', 'image' => null], ['name' => 'Arnetta', 'role' => 'Siswa Jago Belajar', 'quote' => 'Untuk nilai mtk dan bindo nya sama 83.33. Worth mantapp nilainya!', 'image' => null]]); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $testimonial): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <article class="w-full shrink-0 md:w-[calc(50%-10px)] lg:w-[calc(33.333%-13.333px)] overflow-hidden rounded-2xl bg-white shadow-[0_10px_30px_rgba(20,27,44,0.08)] ring-1 ring-[#e9edff]">
                                <div class="relative aspect-[4/3] overflow-hidden bg-[#f1f3ff]">
                                    <img src="<?php echo e(!empty($testimonial['image']) ? asset($testimonial['image']) : asset('images/no-bg-hero.png')); ?>" alt="<?php echo e($testimonial['name']); ?>" class="h-full w-full object-cover">
                                    ...
                                    <div class="absolute bottom-4 left-4 rounded-full bg-[#0043c6] px-3 py-1 text-xs font-bold text-white"><?php echo e($testimonial['name']); ?></div>
                                </div>
                                <div class="p-6">
                                    <p class="text-xs font-semibold uppercase tracking-[0.16em] text-[#64708b]"><?php echo e($testimonial['role']); ?></p>
                                    <p class="mt-3 text-sm leading-7 text-[#434655]">&ldquo;<?php echo e($testimonial['quote']); ?>&rdquo;</p>
                                </div>
                            </article>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
    ...
    <section class="bg-[#f9f9ff] py-12 sm:py-16">
        <div class="jb-container">
            <div class="overflow-hidden rounded-3xl bg-[radial-gradient(circle_at_80%_20%,rgba(30,90,240,0.95),transparent_32%),linear-gradient(135deg,#0043c6,#014fe6)] px-5 py-10 text-center text-white shadow-[0_18px_44px_rgba(0,67,198,0.25)] sm:px-8 sm:py-14">
                <h2 class="text-2xl font-extrabold tracking-tight sm:text-3xl"><?php echo e($homeContent->cta_title ?? 'Siap Raih Skor Terbaikmu?'); ?></h2>
                <p class="mx-auto mt-4 max-w-2xl text-sm leading-7 text-[#edf0ff]"><?php echo e($homeContent->cta_description ?? 'Mulai perjalanan belajarmu bersama Jago Belajar sekarang juga!'); ?></p>
                <div class="mt-8 flex flex-col justify-center gap-4 sm:flex-row">
                    <a href="<?php echo e($homeContent->cta_primary_url ?? route('materials.index')); ?>" class="rounded-xl bg-[#feb700] px-8 py-4 text-sm font-bold text-[#271900] transition hover:-translate-y-0.5 hover:bg-[#ffca35]"><?php echo e($homeContent->cta_primary_label ?? 'Daftar Bimbel Sekarang!'); ?></a>
                    <a href="<?php echo e($homeContent->cta_secondary_url ?? '#program'); ?>" class="rounded-xl border border-white/35 bg-white/10 px-8 py-4 text-sm font-bold text-white transition hover:-translate-y-0.5 hover:bg-white/20"><?php echo e($homeContent->cta_secondary_label ?? 'Lihat Program Bimbel'); ?></a>
                </div>
            </div>
        </div>
    </section>
...
<script>
    function gallerySlider() {
        return {
            active: 0,
            total: <?php echo e(count($galleryItems ?: [['label' => 'BIMBEL ONLINE'], ['label' => 'TRY OUT'], ['label' => 'KELAS SERU']])); ?>,
            timer: null,
            start() {
                this.stop();
    ...
    function testimonialSlider() {
        return {
            active: 0,
            total: <?php echo e(count($testimonialItems ?: [['name' => 'Syafiq'], ['name' => 'Naufal'], ['name' => 'Arnetta']])); ?>,
            visible() {
...
<?php /**PATH C:\dev\jagobelajar-cms\resources\views\pages\public\home.blade.php ENDPATH**/ ?>